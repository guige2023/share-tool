...
const I18N = {
  ...
  zh: {
    ...
    'storage.loadFailed': '加载存储信息失败',
    'notif.failedToLoad': '加载失败',
    ...
    'ui.token': 'Token',
    'ui.sync': '同步',
    'ui.dashboardFiles': '文件',
    'ui.dashboardStorage': '存储',
    'ui.dashboardFavorites': '收藏',
    'ui.dashboardTrash': '回收站',
    'ui.dashboardShares': '分享',
    ...
    'err.openPdfFailed': '打开 PDF 失败',
    'err.openOfficeFailed': '打开 Office 文件失败',
    'err.openMarkdownFailed': '渲染 Markdown 失败',
    'err.openCodeFailed': '打开代码文件失败',
    'err.openCsvFailed': '打开 CSV 失败',
    'err.openArchiveFailed': '打开压缩包失败',
    'err.openEditorFailed': '打开编辑器失败',
    'err.loadFileFailed': '加载文件失败',
    'err.opFailed': '操作失败',
    'err.deleteFailed': '删除失败',
    'err.tagUpdateFailed': '标签更新失败',
    'err.starFailed': '收藏失败',
    'err.folderCreated': '文件夹已创建',
    'err.folderDeleteFailed': '删除文件夹失败',
    'err.noVirtualFolders': '暂无虚拟文件夹',
    'err.loadFoldersFailed': '加载文件夹失败',
  ...
  en: {
    ...
    'storage.loadFailed': 'Failed to load storage info',
    'notif.failedToLoad': 'Failed to load',
...
#vfFilterBtn.active { background: rgba(102, 126, 234, 0.15); border-color: var(--accent-primary); color: var(--accent-primary); }
...
function renderBreadcrumb() {
  ...
  if (currentVfFilter !== null) {
    ...
    fetch(API + '/api/virtual-folders', { headers: { 'x-auth-token': AUTH_TOKEN || '' } })
      .then(r => r.json())
      .then(data => { if (data.success) { const f = data.folders.find(f => f.id === currentVfFilter); if (f) document.getElementById('vfBreadcrumbName').textContent = escapeHtml(f.name); } })
      .catch(() => {});
    return;
  }
  if (!currentFolder) {
...
function showContextMenu(filename, el) {
  ...
  const items = [
    ...
    { label: '📁 ' + T('vf.addFiles'), action: "addToVirtualFolder('" + encodeURIComponent(filename) + "'); hideContextMenu()" },
...
function showFileContextMenu(e, filename) {
  ...
  items.push({ icon: '📁', label: T('vf.addFiles'), action: "addToVirtualFolder('" + encodeURIComponent(filename) + "')" });
...
function closeVfModal() {
  ...
  if (currentVfFilter !== null) {
    ...
    document.getElementById('vfFilterBtn').classList.remove('active');
    loadFiles();
  }
}
...
async function navigateToVf(folderId) {
  closeVfModal(); // close modal first (will check currentVfFilter but it's still null here)
