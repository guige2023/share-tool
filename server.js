...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* Mobile-only share button: hide on desktop */
    .actions-cell button[onclick*="mobileShare"] { display: none; }
    @media (max-width: 600px) {
      .actions-cell button[onclick*="mobileShare"] { display: inline-block; }
    }
    ...
    /* Skeleton loading shimmer for file rows */
    .skeleton-row td { padding: 10px 8px !important; }
    .skeleton-cell {
    ...
    [data-theme="dark"] .skeleton-cell { background: linear-gradient(90deg, #1e293b 25%, #334155 50%, #1e293b 75%); background-size: 200% 100%; }
  ...
  <script>
    ...
    async function loadFilesFromUrl(url, append) {
      ...
      if (append) {
        ...
        if (listBody) {
          var skeletonCount = 8;
          var skeletonHtml = Array.from({length: skeletonCount}, function() {
            return '<tr class="skeleton-row">' +
              ...
              '<td><div class="skeleton-cell" style="width:80px"></div></td>' +
              '<td></td><td><div class="skeleton-cell" style="width:60px"></div></td>' +
              '<td></td>' +
            '</tr>';
          }).join('');
      ...
      modal.innerHTML = '\
        <div class="modal-content" style="max-width:440px">\
          ...
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px">\
            ...
            <button onclick="loadVFSizeAnalysis(' + vfId + ')" id="vfSizeAnalysisBtn" style="font-size:11px;padding:2px 8px;border-radius:6px;cursor:pointer;border:1px solid var(--line);background:var(--bg-secondary);color:var(--text-muted);margin-left:auto" title="查看类型分布和最大文件">📊 分析</button>\
          </div>\
    ...
    window.loadVFSizeAnalysis = async function(vfId) {
    ...
    function renderFiles(tagColorMap) {
      ...
      if (countEl) {
        ...
        if (currentSearchQuery) {
          resultsBar.innerHTML = '<div style="display:flex;align-items:center;gap:8px;flex:1;min-width:0">' +
            '找到 <strong>' + totalLabel + '</strong> 个匹配「<span style="color:var(--accent)">' + escapeHtmlClient(currentSearchQuery) + '</span>」的文件' +
            '</div>' +
            '<button onclick="openTagInputModalForSearchResults()" style="background:var(--accent);border:none;color:#fff;cursor:pointer;font-size:12px;padding:4px 10px;border-radius:6px">' +
              '🏷️ 批量打标签 (' + totalLabel + ')</button>' +
            '<button onclick="clearSearchInput()" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:12px;padding:4px 8px;border-radius:4px">✕ 清除</button>';
